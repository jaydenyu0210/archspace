...
5. Distribution v1: packed tarball into a managed plugins dir; registry later. A **first-party plugin ships as a real plugin** to keep the boundary honest. That turned out to be `plugins/aec-review` (the seven `aec.review.*` nodes), not the IFC plugin this decision first named: IFC arrived as a writer inside `aec.generate_bim_model` (ADR-0016) rather than as a separate process, and the review nodes were the ones with a reason to be isolated. The decision this ADR records — that the boundary is dogfooded by a first-party plugin rather than only offered to third parties — is unchanged.
