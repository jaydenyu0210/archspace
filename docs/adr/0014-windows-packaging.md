...
2. **Unsigned, and said so.** Windows code signing needs an Authenticode certificate from a CA; there is none, and one is not being bought for an alpha. SmartScreen will warn on first run, roughly as Gatekeeper does for the unsigned `.dmg`. `electron-builder` logs `signing with signtool.exe` during the build regardless; that line means it looked, not that it signed. **This decision stands, but the sentence about SmartScreen was wrong about the worst case — see the correction in Consequences.**
...
- **Correction, from the first real attempt to install this (2026-08-27): SmartScreen is not the worst case, and "the user clicks through a warning" was wrong.** On Windows 11 with **Smart App Control** enabled, the installer is refused outright — *"Smart App Control blocked an app that may be unsafe … we can't confirm who wrote it"* — and the dialog offers only **Ok**, **Send feedback** and **Get apps from the Store**. There is no override, hidden or otherwise. Smart App Control is not SmartScreen with a louder voice; it is a different gate that only a trusted signature satisfies, and it is on by default after a clean Windows 11 install. Turning it off is possible but **cannot be reversed without reinstalling Windows**, which is not a thing to ask of someone trying an alpha.

  So for that population — clean Windows 11 installs, which is exactly the newest machines — the unsigned installer is not *inconvenient*, it is **unusable**, and the README said "click More info → Run anyway", advice that does not apply to the box they were looking at. The README now distinguishes the two dialogs and points at running from source, which works because `pnpm dev` launches Electron's own signed binary.

  This raises the priority of signing from "revisit when there is a real audience" to "the Windows download is misleading until it is done".
...
- **Sign with a cheap OV certificate.** OV certificates do not clear SmartScreen reputation immediately either; the warning persists until the certificate accrues reputation. Paying for a warning is not obviously better than an honest unsigned build for an alpha. Revisit with EV or Azure Trusted Signing when there is a real audience. *(Revisited: see the Smart App Control correction above. An OV certificate is still a poor answer to SAC specifically, but the choice is now between paying for signing and shipping a Windows download that a clean Windows 11 machine cannot run at all.)*
