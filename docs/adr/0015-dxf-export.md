...
5. **Text is written as cp1252, with `\U+XXXX` only for what that cannot hold.** R12 predates Unicode: a reader decodes the file in the codepage the header declares, so UTF-8 output arrives as mojibake — `m²` reads as `mÂ²`, `Küche` as `KÃ¼che`. The header declares `$DWGCODEPAGE` = `ANSI_1252` and the bytes are encoded to match. Characters outside cp1252 fall back to `\U+XXXX`, the mechanism AutoCAD itself uses when saving a Unicode drawing back to R12.

   The escape is the fallback rather than the rule because it is not universally decoded — ezdxf returns it as a literal string — whereas a correctly encoded cp1252 byte is right in every reader. So accented Latin, which is the case that actually occurs in room names, survives everywhere; CJK degrades to an escape that is at least legible and obviously an escape. Newlines and other control characters are stripped from values: a newline in a room name shifts every following line onto the wrong side of the code/value pairing and corrupts the file from that byte onward, which readers report as an invalid group code somewhere far from the actual fault.
...
- **A commissioned research pass on the R12 spec and on reader behaviour landed after the first implementation, and changed four things.** It independently reproduced the codepage problem, and recommended cp1252 bytes over a pure-ASCII escape (decision 5 above is the revised version). It established that CRLF is what AutoCAD emits and the safer default, so the file no longer uses LF for intra-project consistency — consistency inside the repository is the wrong thing to optimise for in a file whose purpose is to be opened somewhere else. It found that an undefined symbol is an AutoCAD hard reject while lenient readers substitute silently, which is why `STANDARD` is now defined in a STYLE table alongside `CONTINUOUS` in LTYPE. And it named three silent-failure modes now rejected at the boundary rather than written out: a negative or non-integer layer colour (negative means "layer off", and a decimal in an integer group code makes libdxfrw reject the whole file), a text height of zero (draws nothing, audits clean), and a layer name outside R12's symbol rules. The research is at `docs/research/` alongside the rest.
